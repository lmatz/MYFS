...
using fuse_context   = ::fuse_context;
...
void Log::log_msg(const char *format, ...)
{
    va_list ap;
    va_start(ap, format);
	...
	char *now_str = ctime(&now_t);
	
	fprintf(MYFS_DATA->logfile, "%s     ", now_str);
...
// fuse context
void Log::log_fuse_context(struct ::fuse_context *context)
{
    log_msg("    context:\n");
...
// information in myfs
void Log::log_conn(struct ::fuse_conn_info *conn)
{
    log_msg("    conn:\n");
...
void Log::log_fi (struct ::fuse_file_info *fi)
{
    log_msg("    fi:\n");
...
void Log::log_retstat(char *func, int retstat)
{
    int errsave = errno;
...
int Log::log_syscall(char *func, int retstat, int min_ret)
{
...
void Log::log_stat(struct ::stat *si)
{
    log_msg("    si:\n");
...
void Log::log_statvfs(struct ::statvfs *sv)
{
    log_msg("    sv:\n");
...
void Log::log_utime(struct ::utimbuf *buf)
{
    log_msg("    buf:\n");
